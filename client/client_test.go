...
func testExportAttestations(t *testing.T, sb integration.Sandbox) {
	...
	require.NoError(t, err)
	require.Equal(t, 2, len(index))

	img := index.Find(pk)
	require.NotNil(t, img)
	require.Equal(t, pk, platforms.Format(*img.Desc.Platform))
	require.Equal(t, 1, len(img.Layers))
	require.Equal(t, []byte("hello world!"), img.Layers[0]["greeting"].Data)

	atts := index.Filter("unknown/unknown")
	require.Equal(t, 1, len(atts))
	require.Equal(t, "unknown/unknown", platforms.Format(*atts[0].Desc.Platform))
	require.Equal(t, "unknown/unknown", atts[0].Img.OS+"/"+atts[0].Img.Architecture)
	require.Equal(t, attestation.DockerAnnotationReferenceTypeDefault, atts[0].Desc.Annotations[attestation.DockerAnnotationReferenceType])
	require.Equal(t, img.Desc.Digest.String(), atts[0].Desc.Annotations[attestation.DockerAnnotationReferenceDigest])
	require.Equal(t, 2, len(atts[0].Layers))
	require.Equal(t, len(atts[0].Layers), len(atts[0].Img.RootFS.DiffIDs))
	require.Equal(t, len(atts[0].Img.History), 0)

	var attest intoto.Statement
	require.NoError(t, json.Unmarshal(atts[0].LayersRaw[0], &attest))
	...
	subjects := []intoto.Subject{{
		...
		Digest: map[string]string{
			"sha256": img.Desc.Digest.Encoded(),
		},
	}}
	require.Equal(t, subjects, attest.Subject)

	var attest2 intoto.Statement
	require.NoError(t, json.Unmarshal(atts[0].LayersRaw[1], &attest2))
