...
func testExportAttestations(t *testing.T, sb integration.Sandbox) {
	...
	t.Run("local", func(t *testing.T) {
		...
		for _, p := range ps {
			var attest intoto.Statement
			dt, err := os.ReadFile(path.Join(dir, strings.ReplaceAll(platforms.Format(p), "/", "_"), "attestation.json"))
			require.NoError(t, err)
			...
			require.Equal(t, "https://example.com/attestations/v1.0", attest.PredicateType)
			require.Equal(t, map[string]interface{}{"success": true}, attest.Predicate)
			require.Equal(t, []intoto.Subject{}, attest.Subject)

			var attest2 intoto.Statement
			dt, err = os.ReadFile(path.Join(dir, strings.ReplaceAll(platforms.Format(p), "/", "_"), "attestation2.json"))
			require.NoError(t, err)
